...
@pytest.mark.asyncio
async def test_battery_power_discharge__init(caplog):
    ...
    assert sensor._attr_unique_id == "1234abcd_battery_power_discharge"
...
@pytest.mark.asyncio
async def test_battery_power_discharge__device_info(caplog):
...
@pytest.mark.asyncio
async def test_battery_power_discharge__handle_update_alles_ok_power_groesser_0(caplog):
    # is_pccu_ok(ccu) == true 
...
@pytest.mark.asyncio
async def test_battery_power_discharge__handle_update_alles_ok_aber_batterieleistung_kleiner_0(caplog):
    # is_pccu_ok(ccu) == true 
    # is_power_total_ok(pv_power, batteries) == true
    hass = MagicMock()
    hass.async_add_job = AsyncMock()
...
@pytest.mark.asyncio
async def test_battery_power_discharge__handle_update_pccu_nicht_ok(caplog):
    ...
    with patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.BatteryPowerDischarge.async_write_ha_state", 
               new_callable=MagicMock
               ) as mock_write_ha_state1,\
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_pccu_ok",return_value=False) as mock_is_pccu_ok1,\
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_power_total_ok",return_value=True) as mock_is_power_ok1:
...
@pytest.mark.asyncio
async def test_battery_power_discharge__handle_update_alles_nicht_ok(caplog):
    # is_pccu_ok(ccu) == false 
    ...
    with patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.BatteryPowerDischarge.async_write_ha_state", 
        ...
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_pccu_ok",return_value=False) as mock_is_pccu_ok1,\
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_power_total_ok",return_value=False) as mock_is_power_ok1:
        await sensor1._handle_update(data)
...
@pytest.mark.asyncio
async def test_battery_power_discharge__handle_update_power_total_nicht_ok(caplog):
    # is_pccu_ok(ccu) == true
    # is_power_total_ok(pv_power, batteries) == false
    ...
    with patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.BatteryPowerDischarge.async_write_ha_state", 
        ...
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_pccu_ok",return_value=True) as mock_is_pccu_ok1,\
        patch("custom_components.maxxi_charge_connect.devices.battery_power_discharge.is_power_total_ok",return_value=False) as mock_is_power_ok1:
        await sensor1._handle_update(data)
