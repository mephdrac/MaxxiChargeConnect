...
from custom_components.maxxi_charge_connect.devices.PowerConsumption import (
    PowerConsumption,
)
import pytest
...
@pytest.mark.asyncio
async def test_PowerConsumption_initialization(mock_entry):
    sensor = PowerConsumption(mock_entry)

    assert sensor._attr_unique_id == "test_entry_id_power_consumption"
    assert sensor._attr_icon == "mdi:home-import-outline"
...
@pytest.mark.asyncio
async def test_PowerConsumption_add_and_handle_update1():
    """Test, wenn PccuOk(True) - isPrOk(True)."""
    ...
    with (
        patch(
            "custom_components.maxxi_charge_connect.devices.PowerConsumption.async_dispatcher_connect"
        ) as mock_connect,
        patch(
            "custom_components.maxxi_charge_connect.devices.PowerConsumption.isPccuOk"
        ) as mock_isPccuOk,
        patch(
            "custom_components.maxxi_charge_connect.devices.PowerConsumption.isPrOk"
        ) as mock_isPrOk,
    ):
        mock_isPccuOk.return_value = True
        ...
        pccu = 34.678
        pr = 234.675
        await sensor._handle_update({"Pccu": pccu, "Pr": pr})
        assert sensor.native_value == round(pccu + max(-pr, 0), 2)


@pytest.mark.asyncio
async def test_PowerConsumption_add_and_handle_update2():
    """Test, wenn isPccuOk(False) - isPrOk(True)."""
    ...
    with (
        ...
        mock_isPccuOk.return_value = False
        mock_isPrOk.return_value = True
...
@pytest.mark.asyncio
async def test_PowerConsumption_add_and_handle_update2():
    """Test, wenn isPccuOk(True) - isPrOk(False)."""
...
@pytest.mark.asyncio
async def test_PowerConsumption_add_and_handle_update3():
    """Test, wenn isPccuOk(False) - isPrOk(False)."""
...
@pytest.mark.asyncio
async def test_PowerConsumption_will_remove_from_hass(mock_entry):
    sensor = PowerConsumption(mock_entry)
