...
class PvTotalEnergy(RestoreSensor):
    def __init__(self, entry):
        self._entry = entry
        self._attr_name = "PV Energy Total"
        self._attr_unique_id = f"{entry.entry_id}_pv_energy_total"
    ...
    async def _handle_update(self, data):
        try:
            pv_power_w = float(data.get("PV_power_total", 0))
        ...
        if self._last_update:
            ...
            if 0 < delta < 300:
                self._energy_kwh += (pv_power_w * delta) / 3_600_000
